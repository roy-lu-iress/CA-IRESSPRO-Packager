namespace IressPro.Packager.AppSettings
{
  ...
  {
    public const string BatCtx_CIBC = @"
@echo off
echo Installing IRESS Canada Client Packages with Retail.
echo.
for /R %%R in (*.reg) do REGEDIT.EXE /S %%R
REM REGEDIT.EXE /S Filters.reg
REM REGEDIT.EXE /S OrderPadFilters.reg
REM REGEDIT.EXE /S PortfolioFilters.reg
REM REGEDIT.EXE /S RegTimeZone.reg
Set DFS_DOFMT=FALSE
